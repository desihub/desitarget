...
def remove_fractional_pixels(targs,frac=0.9,nside=64):
    ...
    indsort = np.argsort(targs['HPXPIXEL'])

    #ADM pull out the RAs and Decs, which are the only columns we need other than the grouping column
    ras = targs["RA"][indsort]
    decs = targs["DEC"][indsort]
    pixels = targs['HPXPIXEL'][indsort]

    #ADM find the break points where the array-sorted-by-HPXPIXEL changes to a different pixel
