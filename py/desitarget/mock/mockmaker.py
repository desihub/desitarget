...
class SelectTargets(object):
    ...
    def sample_gmm_nospectra(self, nsample, target='', rand=None):
        """Sample from a Gaussian mixture model generated by gmm-quicksurvey.ipynb.

        Any changes to the quantities fitted in that notebook need to be updated
        here.
...
class ELGMaker(SelectTargets):
    ...
    def make_spectra(self, data=None, indx=None, seed=None, no_spectra=False):
        ...
        if no_spectra:
            ...
            sample = self.sample_gmm_nospectra(nobj, target='ELG', rand=rand)
            meta['MAG'] = sample['r']
            meta['FLUX_G'] = 1e9 * 10**(-0.4 * (sample['g - r'] + sample['r']) )
            meta['FLUX_R'] = 1e9 * 10**(-0.4 * sample['r'])
            meta['FLUX_Z'] = 1e9 * 10**(-0.4 * (sample['r'] - sample['r - z']) )
            meta['FLUX_W1'] = 1e9 * 10**(-0.4 * (sample['r'] - sample['r - z'] - sample['z - W1']) )
