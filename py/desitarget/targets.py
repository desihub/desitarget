...
def initial_priority_numobs(targets):
    ...
    targets : :class:`~numpy.ndarray`
        An array of targets generated by, e.g., :mod:`~desitarget.cuts`
        must include at least (all of) the columns `DESI_TARGET`, `BGS_TARGET` and
        `MWS_TARGET` or the corresponding commissioning or SV columns.
...
def finalize(targets, desi_target, bgs_target, mws_target,
    ...
    targets["PRIORITY_INIT"], targets["NUMOBS_INIT"] = initial_priority_numobs(targets)
