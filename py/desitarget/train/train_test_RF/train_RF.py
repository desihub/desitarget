...
parser.add_argument( '--MODEL', type = str, required = False,
                     default = "DR9s_LOW",
                     help = 'MODEL ID' )
parser.add_argument( '--mod_dpn', type = str,  required = False,
                     default = './WorkingDir/DR9s/RFmodel/DR9s_LOW',
                     help = 'Model directory path name' )
...
for it in range( hyParamSpaceSize ):
    ...
    infoStr = ( "( {:d} ) QSO selected over ( {:d} ) in total for "
        "zred > {:s}" ).format( tmp_n_QSO, n_QSO, str(min_zred) )
    print( infoRootStr + infoStr )
    ...
    RF = RandomForestClassifier( nTrees,
                                 max_depth = maxDepth,
                                 max_leaf_nodes = maxLNodes,
                                 n_jobs = n_jobs )
    # RF training
